...
- Gradio web UI with a real-time chat interface and public share link
...
**Requirements:** Python 3.9+, ~4 GB disk space (for the Phi-2 model download)
...
2. Run all cells in order. The first run will download the Phi-2 model (~5 GB via Hugging Face).

3. The final cells will automatically launch a Gradio web UI:
...
```
dubai-airports-rag/
├── DXB_RAG.ipynb              # Main notebook — full end-to-end pipeline
...
The knowledge base (57 chunks) was extracted from Dubai Airport's internal customer service documentation, split into logical sections, and embedded once — then saved for reuse.

---

## Limitations

- **Phi-2 is small:** May occasionally hallucinate or produce off-topic answers on edge cases
- **Static knowledge base:** The DOCX is not updated automatically; re-run the embedding cells after any document changes
- **No conversation memory:** Each question is handled independently with no multi-turn context
- **English only:** Both the LLM and embeddings are trained on English
...
- Swap Phi-2 for a more capable model (Mistral 7B, LLaMA 3, or a Claude API call)
- Add multi-turn conversation memory
- Support Arabic language queries
- Auto-refresh the knowledge base from a live document source
...
- [Technical Report](docs/technical_report.pdf) — full methodology, evaluation, and results write-up
